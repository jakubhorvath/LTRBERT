...
sequences = [str(rec.seq) for rec in SeqIO.parse("/opt/xhorvat9_TE_DBs/CNN_BERT_train_DB/LTRs_350_700.fasta", "fasta")]
#sequences += [str(rec.seq.complement()) for rec in SeqIO.parse(positive_seq_file, "fasta")]
negative_sequences = [str(rec.seq) for rec in SeqIO.parse("/opt/xhorvat9_TE_DBs/CNN_BERT_train_DB/non_LTRs_350_700.fasta", "fasta")]
#negative_sequences += [str(rec.seq.complement()) for rec in SeqIO.parse(negative_seq_file, "fasta")]


max_len = max([len(seq) for seq in sequences] + [len(seq) for seq in negative_sequences])
oneHotfeatures = []
for se in sequences:
...
if sys.argv[1] == "1":
    wandb.init(project="ComparingLayers_350_700", name="3CNN_large")

    CNN3.fit(trainX, trainY, epochs=15, validation_split=0.2, batch_size=500, callbacks=[WandbCallback()])
elif sys.argv[1] == "2":
    wandb.init(project="ComparingLayers_350_700", name="3CNN_small")

    CNN3.fit(trainX, trainY, epochs=15, validation_split=0.2, batch_size=500, callbacks=[WandbCallback()])
elif sys.argv[1] == "3":
    wandb.init(project="ComparingLayers_350_700", name="3CNN_1LSTM")

    CNN1_LSTM1.fit(trainX, trainY, epochs=15, validation_split=0.2, batch_size=500, callbacks=[WandbCallback()])
elif sys.argv[1] == "4":
    wandb.init(project="ComparingLayers_350_700", name="2CNN_1LSTM")

    CNN2_LSTM1.fit(trainX, trainY, epochs=15, validation_split=0.2, batch_size=500, callbacks=[WandbCallback()])
